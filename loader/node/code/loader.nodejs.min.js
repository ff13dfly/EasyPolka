...
********************** Anchor Network Loader proccess start ***********************`);const fs=require("fs"),args=process.argv.slice(2);if(!args[0])return console.log(config.error,"Error: no input Anchor Link.");const linker=args[0],server=args[1]?args[1]:"ws://127.0.0.1:9944",anchorJS=require("../../package/node/anchor.node"),{ApiPromise,WsProvider}=require("../../package/node/polkadot.node"),{easyRun}=require("../../package/node/easy.node");let websocket=null;const self={auto:r=>{if(websocket!==null)return r&&r();console.log(`Ready to link to server ${server}.`),ApiPromise.create({provider:new WsProvider(server)}).then(e=>(console.log(config.success,`Linker to node [${server}] created.`),websocket=e,anchorJS.set(e),r&&r()))}};console.log(config.success,`Ready to decode Anchor Link : ${linker} .`),self.auto(()=>{const startAPI={common:{latest:anchorJS.latest,target:anchorJS.target,history:anchorJS.history,owner:anchorJS.owner,subcribe:anchorJS.subcribe,block:anchorJS.block}};easyRun(linker,startAPI,result=>{let code=result.code;if(result.libs&&result.libs.order&&result.libs.order.length!==0){const r={};for(let e=0;e<result.libs.order.length;e++){const o=result.libs.order[e],s=`${o[0]}_${o[1]}`;if(result.data[s]){const n=result.data[s],t=new RegExp(`${config.symbol[0]}${n.name}${config.symbol[1]}`,"g");code=code.replace(t,`(function(){${n.raw};return module.exports;})()`)}}}//!important, these functions limit the application
