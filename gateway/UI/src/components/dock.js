...
function Dock(props) {
  ...
  useEffect(() => {
    setInfo(token===null?"No authority to dock vService.":"")
  }, []);
  ...
  return (
    <Row>
      ...
      <Col md={12} lg={12} xl={12} xxl={12} className="pt-2">
        <Form.Control size="md" type="text" disabled={!token} placeholder="vService URI..."
          value={uri}
          onChange={(ev) => { self.onChange(ev) }}
        />
      </Col>
      <Col md={12} lg={12} xl={12} xxl={12} className="pt-2">
        <Form.Control size="md" type="text" disabled={!token} placeholder="******-******-******-******"
          value={secret}
