  ...
  const self={
    status:(evs,amount,from,to)=>{
      ...
      for(let i=0;i<chk_b.length;i++){
        const row=chk_b[i];
        if(row.event.index==="0x0000" && parseInt(row.phase.ApplyExtrinsic)!==0){
          map[row.phase.ApplyExtrinsic]=true;
        }

        if(row.event.index==="0x0602"){
          const data=chk_a[i].event.data;
          if(data[0]===from && data[1]===to && data[2]===amount*1000000000000){
      ...
      RUNTIME.getAccount((fa)=>{
        RUNTIME.getActive((pok)=>{
          ...
          pok.rpc.chain.getBlock(block).then((res)=>{
            const bk=res.toJSON();
            pok.at(block).then((apiAt)=>{
              apiAt.query.system.events().then((res)=>{
                const status=self.status(res,row.amount,fa.address,row.to);
                const confirm = {
                  amount: row.amount,
                  status: "Finalized",
                  to: row.to,
