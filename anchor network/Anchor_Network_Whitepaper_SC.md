...
- **(how,anchor重点)**Anchor Network提供数据操作对象Anchor，这是整个网络的基础，它是一种链上存储方式，用唯一易记的名称来代替随机哈希，建立进入区块链世界的锚，使用类似超链接的锚链接，即可标识Web3.0世界的所有资源。使用JS库AnchorJS，10分钟就可以接入到We3.0的世界，为什么不马上试试呢？

- Anchor Network提供Web3.0的开发协议Easy Protocol，解决链上数据的关系和链上程序的调用方式，可以方便的定位链上资源，同时，通过`申明删除`的方式，来解决数据隐私的问题。这是一个开发可扩展的协议，致力于解决链上数据的关系，支持不同的类型，来实现链上程序、链上数据、链上资源库等开发常用功能。
...
- 简单的入口。Anchor Network使用Anchor来对链上资源进行标定，无论是普通用户还是开发者都可以从中受益。简单的入口，非常便于传播。通过类似URL的链锚，可以对链上的资源和应用进行方便的传播，仅需微小的改动，就可以让现有的应用程序具备访问和写入区块链的能力。

- 简单的经济学。简单化经济学之后，那解决功能性问题，就称为开发者关心的主要方面。Anchor Network的经济学只有`花钱存储`和`自由定价`，前者是存储数据的逻辑，后者是数据交易的逻辑。随着区块链网络的日渐复杂，不同链间的Token流通性无法保障，这对于建立复杂的应用很不利，通过桥等方式，天生的存在响应速度问题。

- 更广泛的去中心化。随着Dapps的增多，Web3.0变得更加中心化了，智能合约在去中心化的单一网络上运行，UI由开发者提供并维护。这些似乎又把Web3.0带回了Web2.0的时代，中心化的应用占据主导，对用户的行为一目了然，例如，中心化的交易所。Anchor Network通过链上发布的方式，来解决这种中心化的趋势，用户无需再借助第三方来维护访问入口。

- 开发语言简单化。Anchor Network希望通过使用Javascript来解决复杂开发的问题，Javacript已经通过它旺盛的生命力说明其本身，那将它转移到区块链的世界，也是合理的选择。虽然，由于其依赖浏览器环境，面临不少的安全问题。Javascript是一个蓬勃发展具有丰富工具链的语言，Anchor Network可以实现完全基于Javascript的全链上系统。
...
- 可以将链锚数据保存为文件进行下载的工具。可用于分析链上数据、链上应用的问题。
