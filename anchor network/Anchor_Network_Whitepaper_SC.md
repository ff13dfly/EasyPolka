...
- 简单的入口。Anchor Network使用Anchor来对链上资源进行标定，无论是普通用户还是开发者都可以从中受益。简单的入口，非常便于传播。通过类似URL的方式，可以对链上的资源和应用进行传播。

- 简单的经济学。简单化经济学之后，那解决功能性问题，就称为开发者关心的主要方面。Anchor Network的经济学只有`花钱存储`和`自由定价`，前者是存储数据的逻辑，后者是数据交易的逻辑。
...
- 开发语言简单化。Anchor Network希望通过使用Javascript来解决复杂开发的问题，Javacript已经通过它旺盛的生命力说明其本身，那将它转移到区块链的世界，也是合理的选择。虽然，由于其依赖浏览器环境，面临不少的安全问题。
